...
#10   12  13   15  16   20   24   25   30   
#   *32
#320  384 416  480 512 640  786  800   960
padheight = 480
padwidth = 640
...
class MyDataset_select(BaseDataset):
# class MyDataset(keras.utils.Sequence):
    ...
    def __init__(
        ...
        if train_flag:
            dataCategory = ("test_nup","baidu_v1","eg1800","icome_task","matting_haf","mydata")
            featureCategory=("big","bighard","negSample","other")   #  ,"little"
            sampleCategory = ("images","masks")
            temp0 = os.listdir(images_dir)
            self.images_fps =[]
            self.masks_fps =[]
            for id0 in dataCategory:
                if id0 in temp0:
                    temp1 = os.listdir(os.path.join(images_dir, id0))
                    for id1 in featureCategory:
                        if id1 in  temp1:
                            img_ids = glob(os.path.join(images_dir,id0,id1, sampleCategory[0],'*.jpg'))
                            self.images_fps +=img_ids
                            temp_fps = list(map(lambda x : x.replace(os.path.join(id0,id1,sampleCategory[0]),os.path.join(id0,id1,sampleCategory[1]))
                             ,img_ids))
                            self.masks_fps+=temp_fps     
        else:
...
#                    print (np.shape(mask0))
                    mask0 = cv2.erode(mask0,self.kernel,iterations=5)
                    mask0 = cv2.dilate(mask0,self.kernel,iterations=4)
...
#                    print (np.shape(mask0))
                    ...
                    mask1 = cv2.erode(mask1,self.kernel,iterations=3)  ####
                    mask1 = cv2.resize(mask1,(w,h))
                    mask0 += mask1  
                #########add20200509
                if random.uniform(0,1.) < 0.3:      
                    mask0 = cv2.erode(mask0,self.kernel,iterations=3)
                if random.uniform(0,1.) < 0.3:
                    mask0 = cv2.dilate(mask0,self.kernel,iterations=3)
...
#            print (np.shape(mask0))
        ...
        if self.portraitNet:
            ...
            if len(mask.shape)==3:
                ...
                mask = mask.copy()
            mask0 =  cv2.erode(mask,self.kernel,iterations=3)
            mask1 =  cv2.erode(mask0,self.kernel,iterations=5)
            mask = mask[None]   #在前面增加一维
            mask0 = mask0[None]
            mask1 = mask1[None]
            return image, mask,mask0,mask1
        else:
            return image, mask
...
if __name__ == "__main__":
    ...
    DATA_DIR = '//SmartGo-Nas/pentao/data/my_cleardata'

    x_train_dir = DATA_DIR#os.path.join(DATA_DIR, 'val/images')
    y_train_dir = DATA_DIR#os.path.join(DATA_DIR, 'val/masks')

    x_valid_dir = os.path.join(DATA_DIR, 'test/val/images')
    y_valid_dir = os.path.join(DATA_DIR, 'test/val/masks')

    x_test_dir = os.path.join(DATA_DIR, 'test/val/images')
    y_test_dir = os.path.join(DATA_DIR, 'test/val/masks')

    portraitNet_flag = True
    augmented_dataset = MyDataset_select(
    x_train_dir, 
    y_train_dir, 
    train_flag= True,
    classes=['person'],
