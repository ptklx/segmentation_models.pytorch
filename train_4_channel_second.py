...
from mydatasets_select_second import MyDataset_select
import platform
...
ENCODER= "mobilenet_v2"  #'resnet18' # densenet121'#'vgg13' #timm-efficientnet-b0'  #"mobilenet_v2"  
...
if winNolinux:
    ...
    init_pre_path='../parameter/seg_pytorch/%s_%s/best_model_last.pth'%(ENCODER,flagt)
    #save_path ='D:/pengt/segmetation/4channels/Unet4/parameter/%s/best_model.pth'%ENCODER
else:
    flagt0="%s"%ENCODER
    flagt="%s_%s"%(ENCODER,'a')
    save_path = '../parameter/seg_pytorch/%s/best_model'%flagt
    save_last_path = '../parameter/seg_pytorch/%s/best_model_last.pth'%flagt
    #pre_path = '../parameter/seg_pytorch/%s/best_model.pth'%ENCODER
    #save_path ='D:/pengt/segmetation/4channels/Unet4/parameter/%s/best_model.pth'%ENCODER
    init_pre_path='../parameter/seg_pytorch/%s/best_model_last.pth'%flagt0
...
train_epoch = smp.utils.train_second.TrainEpoch(
    model, 
    loss=loss, 
    metrics=metrics, 
    optimizer=optimizer,
...
valid_epoch = smp.utils.train_second.ValidEpoch(
    model, 
    loss=loss, 
    metrics=metrics, 
    device=DEVICE,
